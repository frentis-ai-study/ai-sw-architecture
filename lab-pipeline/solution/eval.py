...
def eval_router(suite: EvalSuite) -> None:
    ...
    test_cases = [
        ("연차 잔여일이 몇 일인가요?", "HR", "gpt-5.4-mini"),
        ("재택근무 신청 방법을 알려주세요", "HR", "gpt-5.4-mini"),
        ("복리후생 항목이 뭐가 있나요?", "HR", "gpt-5.4-mini"),
        ("서버 배포 절차를 알려주세요", "IT", "gpt-5"),
        ("장애 대응 매뉴얼은 어디 있나요?", "IT", "gpt-5"),
        ("보안 정책에서 MFA 설정은 필수인가요?", "IT", "gpt-5"),
        ("회의실 예약은 어떻게 하나요?", "일반", "gpt-5.4-mini"),
    ]
...
def eval_semantic_cache(suite: EvalSuite) -> None:
    ...
    # 캐시에 저장
    cache.store("연차 잔여일이 얼마나 남았나요?", "캐시된 응답입니다.", "HR", "gpt-5.4-mini")
